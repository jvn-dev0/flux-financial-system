...
class DatabaseManager:
    ...
    def log_activity(self, account_id, activity_data, risk_score):
        ...
        try:
            df_check = self._load_sheet('ActivityLogs')
            if not df_check.empty and 'AccountID' in df_check.columns and 'Timestamp' in df_check.columns:
                user_logs = df_check[df_check['AccountID'] == account_id]
        ...
        # Build the exact ordered row matching the 21-column schema
        COLUMNS = [
            'LogID', 'AccountID', 'Timestamp', 'TransactionType', 'Description', 'SessionID',
            'TransactionAmount', 'SessionDuration', 'LoginHour', 'FailedLoginCount',
            'NewDeviceLogin', 'PasswordChanged', 'Channel', 'PagesVisited', 'ClickRate',
            'RapidTransactions', 'BeneficiaryAdded', 'LargeTransaction', 'DeviceTrustScore',
            'CyberRiskScore', 'RiskLabel'
        ]

        new_log = {
            'LogID':             f"LOG-{int(now.timestamp())}",
        ...
        if self.use_cloud:
            # Use append_row so we NEVER clear headers
            try:
                ws = self.sh.worksheet('ActivityLogs')
                row_values = [str(new_log.get(col, '')) for col in COLUMNS]
                ws.append_row(row_values, value_input_option='USER_ENTERED')
            except Exception as e:
                print(f"Error appending to ActivityLogs: {e}")
        else:
